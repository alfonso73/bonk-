...
typedef struct _insig
{
    ...
    t_double *g_invec;	    	/* new input samples */
} t_insig;
...
double clock_gettimesince(double prevsystime);
 char *strcpy(char *s1, const char *s2);  // mach-o removed static
static int ilog2(int n);
#endif

 void bonk_tick(t_bonk *x); // mach-o removed static
 ...
 void bonk_tick(t_bonk *x) // mach-o removed static
{
 ...
 void bonk_doit(t_bonk *x)  // mach-o removed static
{
    int i, j, n;
    t_filterkernel *k;
    t_hist *h;
...
static t_int *bonk_perform(t_int *w)
{
 ...
 void bonk_dsp(t_bonk *x, t_signal **sp)  // mach-o removed static
{
 ...
 void bonk_thresh(t_bonk *x, t_floatarg f1, t_floatarg f2)  // mach-o removed static
{
    if (f1 > f2)
 ...
 void bonk_mask(t_bonk *x, t_floatarg f1, t_floatarg f2)  // mach-o removed static
{
    int ticks = f1;
    if (ticks < 0) ticks = 0;
    if (f2 < 0) f2 = 0;
 ...
 void bonk_debounce(t_bonk *x, t_floatarg f1) // mach-o removed static
{
    if (f1 < 0) f1 = 0;
    else if (f1 > 1) f1 = 1;
...
static void bonk_bang(t_bonk *x)
{
 ...
 void bonk_bang(t_bonk *x) // mach-o removed static
{
    bonk_dotick(x, 0);
}

 void bonk_setupkernels(void) // mach-o removed static
{
    int i, j;
    float *fp;
...
{
//    SFTypeList types;   // removed for Mach-O
    short vol = 0;
    OSType type;
    char name[256];
    char **buf;
    ...
    float vec[NFILTERS];
    float *fp, *fp2;
    if (s->s_name[0])  // if it is named, as in "read template".
    {
	vol = defvolume();
	...
	for (eaten = 0; ;)
	    ...
	    x->x_template = (t_template *)t_resizebytes(x->x_template, ntemplate * sizeof(t_template),
	    (ntemplate + 1) * sizeof(t_template));
    ...
    {
	name[0] = 0;
//	types[0]='TEXT';  // removed Mach-O
//  types[1]='maxb';  // removed Mach-O

	open_promptset("Select template for reading.");
    /*	if (open_dialog(name, &vol, &type, 0L, 0))   // if (open_dialog(name, &vol, &type, types, 2))
	{
	    post("bonk~: open canceled");
	    return;
	}*/
 	x->x_template = (t_template *)t_resizebytes(x->x_template, x->x_ntemplate * sizeof(t_template), 0);
...
static void bonk_write(t_bonk *x, t_symbol *s)			// MSP
{
 ...
 void bonk_free(t_bonk *x) // mach-o removed static
{
...
void ext_main()
{
	setup(&bonk_class, bonk_new, (method)bonk_free, (short)sizeof(t_bonk), 0L, A_DEFLONG, A_DEFLONG, 0);
